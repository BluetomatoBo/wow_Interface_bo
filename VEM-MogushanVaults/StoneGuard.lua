local mod	= VEM:NewMod(679, "VEM-MogushanVaults", nil, 317)
...
local function 	ChecknextOverload()
	...
	if mod.Options.InfoFrame then
		VEM.InfoFrame:SetHeader(NOSTAT.."  "..SDSTAT)
		VEM.InfoFrame:Show(4, "cobalypower", SDNOW)
	end
	if mod.Options.AInfoFrame then
		VEM.InfoFrame:SetHeader(SDSTAT)
		VEM.InfoFrame:Show(4, "cobalypower", SDNOW, true)
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(130395) then
		...
		if activePetrification ~= "Jasper" then
			if self.Options.ArrowOnJasperChains and #jasperChainsTargets == 2 then
				if jasperChainsTargets[1] == UnitName("player") then
					VEM.Arrow:ShowRunTo(jasperChainsTargets[2])
				elseif jasperChainsTargets[2] == UnitName("player") then
					VEM.Arrow:ShowRunTo(jasperChainsTargets[1])
				end
			end
		end
		if args:IsPlayer() then
			...
			if uId and (UnitPower(uId) <= 80) and (activePetrification == "Jasper") then--Make sure his energy isn't already high, otherwise breaking chains when jasper will only be active for a few seconds is bad
				...
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_ldsl.mp3") --拉斷鎖鏈
				VEM.Arrow:Hide()
			else
				specWarnJasperChains:Show()
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_lx.mp3")--連線快靠近
			end
		end
	...
	elseif args:IsSpellID(115745) then
		...
		if args.sourceGUID == UnitGUID("target") then
			if mod:IsTank() then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_mbsh.mp3")--目標石化
			end
		end
	end
end
...
function mod:RAID_BOSS_EMOTE(msg, boss)
	...
	elseif msg:find("spell:116529") then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_dzcz.mp3")--地磚重置
	end
end
...
function mod:OnSync(msg, boss)
	...
	if msg == "Overload" and self:AntiSpam(2, 6) then
		...
		if boss == "Cobalt" then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_lscz.mp3") --藍色超載		
		elseif boss == "Jade" then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_lvscz.mp3") --綠色超載
		elseif boss == "Jasper" then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_hscz.mp3") --紅色超載
		elseif boss == "Amethyst" then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_zscz.mp3") --紫色超載
		end
		ChecknextOverload()
	end
end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 115852 then
		...
		warnBSD:Show()
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_lssh.mp3") --藍色石化
		SDSTAT = L.SDBLUE		
		ChecknextOverload()
	...
	elseif spellId == 116006 then
		...
		timerPetrification:Start()
		warnGSD:Show()
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_lvssh.mp3") --綠色石化
		SDSTAT = L.SDGREEN
		ChecknextOverload()
	...
	elseif spellId == 116036 then
		...
		timerPetrification:Start()
		warnRSD:Show()
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_hssh.mp3") --紅色石化
		SDSTAT = L.SDRED
		ChecknextOverload()
		...
		if playerHasChains then
			...
			if uId and (UnitPower(uId) <= 80) and (activePetrification == "Jasper") then
				specWarnBreakJasperChains:Show()
				sndWOP:Schedule(1, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_ldsl.mp3") --拉斷鎖鏈
				VEM.Arrow:Hide()
			end
		end
	elseif spellId == 116057 then
		...
		timerPetrification:Start()
		warnPSD:Show()
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_zssh.mp3") --紫色石化
		SDSTAT = L.SDPURPLE
		ChecknextOverload()
...
function mod:GetHP()
	...
	return cobaltHp ~= VEM_CORE_UNKNOWN and cobaltHp or jadeHp
end
