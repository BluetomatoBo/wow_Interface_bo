local mod	= VEM:NewMod(826, "VEM-Pandaria", nil, 322)
...
--		timerCrushCD:Start(-delay)--There was no tank, so he pretty much never cast this, just ran like a wild animal around area while corpse cannoned
	...
	if self.Options.RangeFrame then
		VEM.RangeCheck:Show(10)--range is guessed. spell tooltip and EJ do not save what range is right now.
	end
end
...
function mod:SPELL_CAST_START(args)
	...
	elseif args.spellId == 137505 then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\watchwave.mp3") --衝擊波
	end
end
