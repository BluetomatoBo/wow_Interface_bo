local mod	= VEM:NewMod(691, "VEM-Pandaria", nil, 322)	-- 322 = Pandaria/Outdoor I assume
...
function mod:updateRangeFrame()
	...
	if UnitDebuff("player", GetSpellInfo(119622)) then
		VEM.RangeCheck:Show(5, nil)--Show everyone.
	else
		VEM.RangeCheck:Show(5, debuffFilter)--Show only people who have debuff.
	end
end
...
function mod:SPELL_CAST_START(args)
	if args.spellId == 119488 then
		...
		sndWOP:Schedule(2, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\aesoon.mp3") --AE
...
function mod:SPELL_AURA_APPLIED(args)
	if args.spellId == 119622 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\findmc.mp3") --心控
		...
		if self.Options.HudMAP then
			MCMarkers[args.destName] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("timer", args.destName, 5, nil, 0, 1, 0, 1):Appear():RegisterForAlerts():Rotate(360, 6))
		end
...
function mod:SPELL_AURA_REMOVED(args)
	if args.spellId == 119626 and self.Options.SetIconOnMC then--Remove them after the MCs break.
		removeIcon(VEM:GetRaidUnitId(args.destName))
		if args:IsPlayer() then
			playerMCed = false
		end
