...
function VUF:UpdateElementAnchor(frame,element)
	...
	local enableAuraBars = VUF.enableAuraBars
	...
	if element == 'mushroom' then
		...
		WMFrame:SetScript('OnEvent',function(self,event)
			local config = E.db.unitframe.vuf.units[frame.unit]['mushroom']
			local anchor = config['anchor']
			local eclipse
			local spec = GetSpecialization()
			if spec == 1 then
			...
			VUF:CheckHealthValue(frame,eclipse)
		end)
		local spec = GetSpecialization()
		if spec == 1 then
			anchor = anchor['eclipse']
...
function VUF:PostUpdateHealth(unit, min, max)
	...
	if E.db.unitframe.vuf.units[unit].portrait and E.db.unitframe.vuf.units[unit].portrait.enabled then
	...
	if (min / max * 100) < (E.db.unitframe.vuf.lowThreshold) then
		E:Flash(parent, 0.6)
		if (not warningTextShown and unit == "player") and E.db.unitframe.vuf.warningText then
			ElvUIVerticalUnitFramesWarning:AddMessage("|cffff0000LOW HEALTH")
			warningTextShown = true
		else
			ElvUIVerticalUnitFramesWarning:Clear()
			warningTextShown = false
		end
		if unit == "player" and E.db.unitframe.vuf.screenflash then
			ElvUIVerticalUnitFramesScreenFlash:SetAlpha(1)
			E:Flash(ElvUIVerticalUnitFramesScreenFlash, 0.6)
		end
	else
		E:StopFlash(parent)
		E:StopFlash(ElvUIVerticalUnitFramesScreenFlash)
		ElvUIVerticalUnitFramesScreenFlash:SetAlpha(0)
	end
end
...
function VUF:PostUpdatePowerVerticalUnitFrame(unit, min, max)
	-- Flash mana below threshold %
	local powerMana, _ = UnitPowerType(unit)
	if (min / max * 100) < (E.db.unitframe.vuf.lowThreshold) and (powerMana == SPELL_POWER_MANA) and VUF.db.flash then
		if VUF.db.warningText then
			if not warningTextShown and unit == "player" then
				ElvUIVerticalUnitFramesWarning:AddMessage("|cff00ffffLOW MANA")
				warningTextShown = true
			else
				ElvUIVerticalUnitFramesWarning:Clear()
				warningTextShown = false
...
function VUF:ComboDisplay(event, unit)
...
function VUF:PostCastStart(unit, name, rank, castid)
...
function VUF:PostUpdateAura(unit, icon, index, offset, filter, isDebuff, duration, timeLeft)
...
function VUF:AuraFilter(unit, icon, name, rank, texture, count, dtype, duration, timeLeft, caster, isStealable, shouldConsolidate, spellID, canApplyAura, isBossDebuff)	
...
function VUF:AuraBarFilter(unit, name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate)
	if unit == "vehicle" then unit = "player" end
	if not E.db.unitframe.units[unit] then return end
...
function VUF:PortraitUpdate(unit)
	if unit == "vehicle" then unit = "player" end
	local db = E.db.unitframe.vuf.units[unit]
...
function VUF:PostUpdateShadowOrbBar(event)
	local spec = GetSpecialization()
