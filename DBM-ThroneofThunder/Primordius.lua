...
--	print("DBM Debug Spawn: ", bigOozeAlive)
	...
	if mod:IsTank() then
		sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\ex_tt_drn.mp3")--大軟泥
	end
...
function mod:SPELL_CAST_START(args)
	if args.spellId == 136216 then
		...
		sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\ex_tt_kjfd.mp3")--靠近分擔
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args.spellId == 136246 then
		...
		sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\range2.mp3")--2碼手雷
		showspellinfo()
	...
	elseif args.spellId == 136228 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\holdit.mp3")--自保
			DBM.Flash:Shake(1, 0, 0)
		elseif mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\ex_tt_byt.mp3")--病原體出現
		end
	elseif args.spellId == 136245 then
		metabolicBoost = true
		sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\ex_tt_sljs.mp3")--加速
	...
	elseif args.spellId == 136218 then
		...
		sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\range5.mp3")--注意分散
		showspellinfo()
	elseif args.spellId == 140546 and args:IsPlayer() then
		specWarnFullyMutated:Show()
		DBM.Flash:Shake(0, 1, 0)
		timerFullyMutated:Start()
		sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\ex_tt_tbwc.mp3")--完美突變
	end
end
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args.spellId == 140546 and args:IsPlayer() then
		...
		sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\ex_tt_bsjs.mp3")--變身結束
	end
end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 136248 and self:AntiSpam(2, 1) then--Pustule Eruption
		...
		if not self:IsDifficulty("lfr25") then
			...
			sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\ex_tt_zynx.mp3")--注意膿血
