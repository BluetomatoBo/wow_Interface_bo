...
-- Internal Data
local Plates, PlatesVisible, GUID = {}, {}, {}	            	-- Plate Lists
...
do
	...
	function OnUpdate(self, e)
		...
		for plate in pairs(PlatesVisible) do
			...
			-- Alpha Animation
			local req, vis = extended.requestedAlpha, extended.visibleAlpha
			if req ~= vis then
				if req > vis then
					vis = min(vis + .05, req)
				elseif req < vis then
			...
			-- Scaling Animation
			local req, vis = extended.requestedScale, extended.visibleScale
			if req ~= vis then
				if req > vis then
					vis = min(vis + .05, req)
				elseif req < vis then
				...
				extended.visibleScale = vis
				extended:SetScale(extended.visibleScale)
			end

			-- Color Animation
			local visR, visG, visB = extended.visibleR , extended.visibleG , extended.visibleB
			local reqR, reqG, reqB = extended.requestedR , extended.requestedG, extended.requestedB
			local diffR, diffG, diffB = reqR - visR, reqG - visG, reqB - visB

			if not (diffR == 0 and diffG == 0 and diffB == 0 ) then

				if diffG > 0 then
					visG = min(visG + .05, reqG)
				elseif diffG < 0 then
					visG = max(visG - .05, reqG)
				end

				if diffR > 0 then
					visR = min(visR + .05, reqR)
				elseif diffR < 0 then
					visR = max(visR - .05, reqR)
				end

				if diffB > 0 then
					visB = min(visB + .05, reqB)
				elseif diffG < 0 then
					visB = max(visB - .05, reqB)
				end

				extended.visual.healthbar:SetStatusBarColor(visR, visG, visB)
				extended.visibleR , extended.visibleG , extended.visibleB = visR, visG, visB
				--visual.healthbar:SetStatusBarColor
			end


			-- Health Bar Animation
			local req, vis, mx = extended.requestedHealth, extended.visibleHealth, extended.unit.healthmax
			if req ~= vis then
				if req > vis then
					vis = min(vis + mx, req)
				elseif req < vis then
					vis = max(vis - (mx/100), req)
				end

				extended.visibleHealth = vis
				extended.visual.healthbar:SetValue(vis)
			end
...
do
	...
	function OnShowNameplate(plate)
		...
		-- Scale
		extended:SetScale(.7)
		extended.visibleScale = .7
		extended.requestedScale = .7

		-- Health
		extended.requestedHealth = 1
		extended.visibleHealth = 0
		extended.visibleR , extended.visibleG , extended.visibleB = .2,.2,.2
		extended.requestedR , extended.requestedG, extended.requestedB = 1,1,1
...
do
	...
	function UpdateIndicator_HealthBar()
		...
		extended.requestedHealth = bars.health:GetValue()
	end
	...
	function UpdateIndicator_UnitColor()
		...
		if activetheme.SetHealthbarColor then
			...
			extended.requestedR , extended.requestedG, extended.requestedB = activetheme.SetHealthbarColor(unit)
	...
	function UpdateIndicator_CustomScaleText()
		...
		if unit.health and (extended.requestedAlpha > 0) then
			...
			if activetheme.SetScale then
				local previousScale = extended.requestedScale
				-- scale = activetheme.SetScale(unit)
				extended.requestedScale = activetheme.SetScale(unit) or previousScale or 1
