...
--]]
    ...
    --[[
    Replace:
    UpdateIndicator_UpdateTargetCastbar
...
do
	...
	function ApplyPlateExtension(plate)
		...
		regions.threatglow, regions.healthborder, regions.highlight, 
			regions.level, regions.skullicon, regions.raidicon, 
			regions.eliteicon = bargroup:GetRegions()
			
		regions.name = namegroup:GetRegions()
		
		regions.castborder, regions.castnostop, regions.spellicon,
			regions.spelltext, regions.spellshadow =  select(2, cast:GetRegions())
...
TidyPlates.StartCastAnimationOnNameplate = function(...) end
TidyPlates.StopCastAnimationOnNameplate = function(...) end
TidyPlates.StartSpellCastWatcher = function(...) end
TidyPlates.StopSpellCastWatcher = function(...) end
