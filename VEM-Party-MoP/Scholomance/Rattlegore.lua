local mod	= VEM:NewMod(665, "VEM-Party-MoP", 7, 246)
...
function mod:OnCombatStart(delay)
	...
	if not UnitDebuff("player", boned) then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\getboned.mp3")--快拿骨甲
	end
	if self.Options.InfoFrame then
		VEM.InfoFrame:SetHeader(L.PlayerDebuffs)
		VEM.InfoFrame:Show(5, "playergooddebuff", 113996)
	end
end
