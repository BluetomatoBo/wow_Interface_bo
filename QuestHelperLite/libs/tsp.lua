-- Modified for use with QuestHelperLite by WakeZero
----------------------------------
--[[
Ant Colony Optimization (ACO) for Travelling Salesman Problem (TSP)
taken from Routes (a World of Warcraft addon)
...
-- in mind and have been modified for use with QuestHelper Lite. It is not
-- intended for use as a general TSP library.
...
TSP.parameters = {
	...
	beta               = 3,     -- Likelihood of ants to choose closer nodes (larger value == more likely)
...
function TSP:getID(x, y)
	if x and y then return floor(x * 10000 + 0.5) * 10000 + floor(y * 10000 + 0.5) end
end
function TSP:getXY(id)
	if id then return floor(id / 10000) / 10000, (id % 10000) / 10000 end
end
...
function TSPUpdateFrame:OnUpdate(elapsed)
	...
	if status then
		...
		self.finishFunc = nil
		self.nodes = nil
		--QHL:Print("The following error occured in the background path generation coroutine:")
		--QHL:Print(path)
	end
end
...
function TSP:SolveTSPBackground(nodes, metadata, taboos, zoneW, zoneH, parameters, path)
	if not TSPUpdateFrame.running then
		...
		local status = coroutine.resume(TSPUpdateFrame.co, TSP, nodes, metadata, taboos, zoneW, zoneH, parameters, path, true)
		if status then
...
-----------------------------------
-- TSP:SolveTSP(nodes, metadata, zoneW, zoneH, parameters, path, nonblocking)
-- Arguments
...
--   zoneW,zoneH - The width and height of the path zone
...
function TSP:SolveTSP(nodes, metadata, taboos, zoneW, zoneH, parameters, path, nonblocking)
	...
	if not parameters then parameters = self.parameters; end
	...
	if numNodes < 4 then
		...
		return path, metadata2, TSP:PathLength(path, zoneW, zoneH), 0, 0
	end
	--]]
	...
	if numNodes < 4 then
		...
		path[1] = nodes[1]
		if metadata then
			metadata2[1] = newTable()
			for j = 1, #metadata[1] do
				metadata2[1][j] = metadata[1][j]
			end
		end
	
		-- Find closest node
		local x2, y2, x3, y3, d2, d3;
		local x, y = floor(nodes[1] / 10000) / 10000, (nodes[1] % 10000) / 10000
		if nodes[2] then
			x2, y2 = floor(nodes[2] / 10000) / 10000, (nodes[2] % 10000) / 10000
			d2 = (((x2 - x)*zoneW)^2 + ((y2 - y)*zoneH)^2)^0.5
		end
		if nodes[3] then
			x3, y3 = floor(nodes[3] / 10000) / 10000, (nodes[3] % 10000) / 10000
			d3 = (((x3 - x)*zoneW)^2 + ((y3 - y)*zoneH)^2)^0.5
		end
		if d2 and (not d3 or d2 <= d3) then 
			path[2] = nodes[2];
			if metadata then
				metadata2[2] = newTable()
				for j = 1, #metadata[2] do
					metadata2[2][j] = metadata[2][j]
				end
			end
			if nodes[3] then 
				path[3] = nodes[3]
				if metadata then
					metadata2[3] = newTable()
					for j = 1, #metadata[3] do
						metadata2[3][j] = metadata[3][j]
					end
				end
			end
		elseif d3 and (not d2 or d3 < d2) then 
			path[2] = nodes[3];
			if metadata then
				metadata2[2] = newTable()
				for j = 1, #metadata[3] do
					metadata2[2][j] = metadata[3][j]
				end
			end
			if nodes[2] then 
				path[3] = nodes[2]
	...
	local numAnts           = numNodes --ceil(2 * numNodes ^ 0.5)              -- Parameter: Number of ants.
	...
	while nochanges < MAXUNCHANGEDINTERATION do
		...
		for k = 1, numAnts do
			...
			local pathLength = 0
			curnode = antpath[1] --curnode = antpath[numNodes]
			for i = 2, numNodes do --for i = 1, numNodes do
				nextnode = antpath[i]
		...
		local curnode = shortestPath[1] --local curnode = shortestPath[numNodes]
	...
	shortestPathLength = TSP:PathLength(path, zoneW, zoneH)
...
function TSP:TwoOpt(path, weight, prune, twoPointFiveOpt, nonblocking)
	...
	-- MODIFIED FOR NO RETURN TRIP
	for i = 2, numNodes-3 do --for i = 1, numNodes-3 do
	...
	if twoPointFiveOpt then
		...
		-- MODIFIED FOR NO RETURN TRIP
		for i = 2, numNodes-4 do --for i = 1, numNodes-4 do
...
-- TSP:InsertNode(nodes, zoneW, zoneH, nodeID, twoOpt, path)
...
--        directly
function TSP:InsertNode(nodes, metadata, zoneW, zoneH, nodeID, radius)
	...
	if numNodes < 3 then
		...
		return TSP:PathLength(nodes, zoneW, zoneH)
	end
...
-- TSP:PathLength(nodes, zoneW, zoneH)
...
-- Returns
--   pathLength - The length of the route in yards.
function TSP:PathLength(nodes, zoneW, zoneH)
	...
	local x2, y2 = floor(nodes[1] / 10000) / 10000, (nodes[1] % 10000) / 10000 
	for i = 2, #nodes do --for i = 1, #nodes do
...
-- TSP:ClusterRoute(nodes, zoneW, zoneH, radius)
-- Arguments
--   nodes    - The table containing a list of Routes node IDs to path
--              This list should only contain nodes on the same map. This
--              table should be indexed numerically from nodes[1] to nodes[n].
...
function TSP:ClusterRoute(nodes, zoneW, zoneH, radius)
