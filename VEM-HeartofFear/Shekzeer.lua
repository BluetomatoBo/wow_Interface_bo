local mod	= VEM:NewMod(743, "VEM-HeartofFear", nil, 330)
...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(123707) then
		...
		if args:IsPlayer() and (args.amount or 1) >= 4 then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_csgg.mp3") --層數過高
		else
	...
	elseif args:IsSpellID(123788) then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_kjyy.mp3") --快進音域
		end
	...
	elseif args:IsSpellID(124862) then
		...
		if args:IsPlayer() then
			...
			sndWOP:Schedule(1.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndWOP:Schedule(2.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			sndWOP:Schedule(3.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		end
		if self:AntiSpam(5, 3) then
			...
			if MyTT() then
				...
				sndWOP:Schedule(1, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_zlzb.mp3") --戰慄準備
				sndWOP:Schedule(1.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
				sndWOP:Schedule(2.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
				sndWOP:Schedule(3.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			end
		...
		if mod:IsHealer() then
			sndWOP:Schedule(4, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\dispelnow.mp3")
		end
		self:Unschedule(warnVisionsTargets)
		self:Schedule(0.3, warnVisionsTargets)
		if args:IsPlayer() then
			if self.Options.HudMAP then
				DeadMarkers[args.destName] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("reticle", args.destName, 9, nil, 0, 1, 0, 0.5):Appear():RegisterForAlerts():Rotate(360, 4))
			end
		else	
			if self.Options.HudMAPOther then
				DeadMarkers[args.destName] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("ring", args.destName, 9, nil, 0, 1, 0, 1):Appear():RegisterForAlerts())
			end
		end
	...
	elseif args:IsSpellID(124097) then
		if args:IsPlayer() and self:AntiSpam(5, 2) then --prevent spam in heroic
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_szkp.mp3") --樹脂
		end
	...
	elseif args:IsSpellID(124821) then
		...
		if self.Options.HudMAP2 then
			QJMarkers[args.destName] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("timer", args.destName, 2, nil, 0, 1, 0, 1):Appear():RegisterForAlerts():Rotate(360, 120))
		end
	...
	elseif args:IsSpellID(123845) then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_lxkp.mp3")--連線快跑
		end
	end
end
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args:IsSpellID(125390) then
		if args:IsPlayer() then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\safenow.mp3") --安全
		end
	elseif args:IsSpellID(124097) then
		if self.Options.StickyResinIcons then
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(125826) then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_xjwc.mp3") --陷阱完成
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(124849) then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_kjts.mp3")--恐懼吞噬
	end
end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 125098 then--Yell is about 1.5 seconds faster then this event, BUT, it also requires localizing. I don't think doing it this way hurts anything.
		...
		sndWOP:Schedule(152, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_nwzb.mp3") --女王準備
		sndWOP:Schedule(153, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
		sndWOP:Schedule(154, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
		sndWOP:Schedule(155, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		sndWOP:Schedule(156, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		if self.Options.InfoFrame then
			VEM.InfoFrame:SetHeader(L.PlayerDebuffs)
			VEM.InfoFrame:Show(10, "playerbaddebuff", 125390)
		end
		if self.Options.RangeFrame then
			VEM.RangeCheck:Hide()
		end
	elseif spellId == 125304 then
		...
		sndWOP:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_nwzb.mp3")
		...
		ptwo = false
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\phasechange.mp3")--階段轉換
		sndWOP:Schedule(147, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ptwo.mp3")--P2
		sndWOP:Schedule(148, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
		sndWOP:Schedule(149, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
		sndWOP:Schedule(150, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		sndWOP:Schedule(151, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		...
		if UnitDebuff("player", GetSpellInfo(123184)) then
			if self:IsDifficulty("heroic10", "heroic25") then
				if UnitHealth(uId) / UnitHealthMax(uId) <= 0.8 and not warnedhp then
					sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\checkhp.mp3")--注意血量
					warnedhp = true
				end
			else
				if UnitHealth(uId) / UnitHealthMax(uId) <= 0.5 and not warnedhp then
					sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\checkhp.mp3")
					warnedhp = true
				end
			end
		else
...
function mod:OnSync(msg, guid, hp)
	if msg == "ybhealth" and guid and hp then
		...
		if self.Options.InfoYB then
			VEM.InfoFrame:SetHeader(GetSpellInfo(123184))
			if #sh == 2 then
				VEM.InfoFrame:Show(1, "other", sh[2].V, sh[1].V)
			elseif #sh == 1 then
				VEM.InfoFrame:Show(1, "other", "-", sh[1].V)
			end
		end
		...
		if warnhp < 11 and not warnedLowHP[guid] then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_ybbz.mp3") --音波爆炸準備
		end
		if warnhp == 5  and not warnedLowHP5[guid] then
			warnedLowHP5[guid] = true
			sndYB:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
		end
		if warnhp == 4  and not warnedLowHP4[guid] then
			warnedLowHP4[guid] = true
			sndYB:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
		end
		if warnhp == 3  and not warnedLowHP3[guid] then
			warnedLowHP3[guid] = true
			sndYB:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
		end
		if warnhp == 2  and not warnedLowHP2[guid] then
			warnedLowHP2[guid] = true
			sndYB:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		end
		if warnhp == 1  and not warnedLowHP1[guid] then
			warnedLowHP1[guid] = true
			sndYB:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		end
	elseif msg == "swpos" and guid and hp then
		if guid == UnitName("player") then
			if self.Options.AcceptPos then
				DeadEg = register(VEMHudMap:PlaceRangeMarker("timer", RunPos[hp][1], RunPos[hp][2], 2, 5, 0, 1, 0, 1):Appear():RegisterForAlerts())
				DeadEg = register(VEMHudMap:AddEdge(0, 0, 1, 1, 5, "player", nil, nil, nil, RunPos[hp][1],RunPos[hp][2]))
			end
		end
	end
end
...
function mod:RAID_BOSS_EMOTE(msg)
	if msg:find("spell:126121") then
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_yyfh.mp3") --音域腐化
	end
end