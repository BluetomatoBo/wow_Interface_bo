local mod	= VEM:NewMod(737, "VEM-HeartofFear", nil, 330)
...
local function updateInfoFrame()
	if mod.Options.optInfoFrame == "IF1" then
		if Phase == 2 then
			VEM.InfoFrame:SetHeader("Yellow Boss Destabilize:"..boss2debuff)
		else
			VEM.InfoFrame:SetHeader("Boss Destabilize:"..bossdebuff)
		end
		VEM.InfoFrame:Show(5, "playerpower", 1, ALTERNATE_POWER_INDEX, nil, nil, true)--At a point i need to add an arg that lets info frame show the 5 LOWEST not the 5 highest, instead of just showing 10
	elseif mod.Options.optInfoFrame == "IF2" then
		if otherconstruct and (Phase < 3) then
			VEM.InfoFrame:SetHeader("My Power:"..UnitPower("player", ALTERNATE_POWER_INDEX).."  Others Power"..UnitPower(otherconstruct, ALTERNATE_POWER_INDEX))
		else
			VEM.InfoFrame:SetHeader("My Power:"..UnitPower("player", ALTERNATE_POWER_INDEX))
		end
		VEM.InfoFrame:Show(2, "bossdebuffstacks", 123059)
	end
end
...
function mod:ScalpelTarget()
	...
	local targetname = VEM:GetUnitFullName("boss1targettarget")--Not a mistake, just clever use of available api to get the target of an invisible mob the boss is targeting ;)
...
local function warnAmberExplosionCast(spellId)
	if #canInterrupt == 0 then--This will never happen if fired by "InterruptAvailable" sync since it should always be 1 or greater. This is just a fallback if contructs > 0 and we scheduled "warnAmberExplosionCast" there
		...
		if spellId == 122398 then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_hpbz.mp3") --琥珀爆炸
		end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(121949) then
		...
		if mod:IsHealer() and (not playerIsConstruct) then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_jscz.mp3") --寄生成長
		end
	elseif args:IsSpellID(122540) then
		...
		sndWOP:Schedule(48.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countseven.mp3")
		sndWOP:Schedule(49.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countsix.mp3")
		sndWOP:Schedule(50.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
		sndWOP:Schedule(51.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
		sndWOP:Schedule(52.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
		sndWOP:Schedule(53.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		sndWOP:Schedule(54.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
	...
	elseif args:IsSpellID(122784) then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_nbzh.mp3") --你被轉化
	...
	elseif args:IsSpellID(125502) then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\orbrun.mp3") --寶珠快跑
		end
	end
end
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args:IsSpellID(122540) then--Phase 3
		...
		sndWOP:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countseven.mp3")
		sndWOP:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countsix.mp3")
...
function mod:SPELL_CAST_START(args)
	...
	elseif args:IsSpellID(122402) then--Amber Monstrosity
		...
		warnAmberExplosion:Show(args.sourceName, args.spellName)
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_jgbz.mp3") --巨怪爆炸
		warnAmberExplosionSoon:Cancel()
		warnAmberExplosionSoon:Schedule(41)
		timerAmberExplosion:Start()
		timerAmberExplosionAMCD:Start(46, args.spellName)
		sndWOP:Schedule(39, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countseven.mp3")
		sndWOP:Schedule(40, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countsix.mp3")
		sndWOP:Schedule(41, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
		sndWOP:Schedule(42, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
		sndWOP:Schedule(43, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
		sndWOP:Schedule(44, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		sndWOP:Schedule(45, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		self:Unschedule(warnAmberExplosionCast)
	...
	elseif args:IsSpellID(122408) then
		if not playerIsConstruct then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\stompsoon.mp3") --準備踐踏
		end
...
function mod:UNIT_POWER(uId)
	...
	elseif playerWill < 18 and not warnedWill then--5 seconds before 0 (after subtracking a budget of 8 for interrupt)
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_yzgd.mp3") --意志過低
...
function mod:SWING_DAMAGE(sourceGUID, _, _, _, destGUID)
	...
	if cid == 62691 and destGUID == UnitGUID("player") and not warnedoo[sourceGUID] and not playerIsConstruct then
		sndADD:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\didi.mp3")
		specwarnOOYou:Show()
