...
local function warnMutatedTargets()	
	...
	if mod:AntiSpam(5, 1) then
		...
		if mutatecount == 1 then
			sndWOP:Schedule(0.6, "Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\countone.mp3")
		elseif mutatecount == 2 then
			sndWOP:Schedule(0.6, "Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\counttwo.mp3")
		elseif mutatecount == 3 then
			sndWOP:Schedule(0.6, "Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\countthree.mp3")
		elseif mutatecount == 4 then
			sndWOP:Schedule(0.6, "Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\countfour.mp3")
		elseif mutatecount == 5 then
			sndWOP:Schedule(0.6, "Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\countfive.mp3")
		elseif mutatecount == 6 then
			sndWOP:Schedule(0.6, "Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\countsix.mp3")
		end
		...
		if UnitExists(unitID) and mod:GetCIDFromGUID(UnitGUID(unitID)) == 71161 then
			if UnitExists(unitID.."target") and not mod:IsTanking(unitID.."target", unitID) then
				...
				if mod.Options.HudMAPDF then					
					DFMarker = register(DBMHudMap:PlaceStaticMarkerOnPartyMember("highlight", targetname, 5, 3, 0, 1, 0, 0.8):Appear():RegisterForAlerts())
				end
			else
...
function mod:SPELL_CAST_START(args)
	...
	elseif args.spellId == 142728 then
		...
		sndWOP:Schedule(2, "Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\ex_so_bzhh.mp3") --爆炸火環準備
	...
	elseif args.spellId == 142730 then
		...
		sndWOP:Schedule(1, "Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\ex_so_xxls.mp3") --小心綠水
	...
	elseif args.spellId == 142416 then
		...
		if MyJS() then
			sndWOP:Schedule(0.8, "Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\defensive.mp3") --注意減傷
			sndWOP:Schedule(1.5, "Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\defensive.mp3")
		end
