...
-- Special thanks to:
--    * Arta (VEM-Party)
--    * Omegal @ US-Whisperwind (some patches, and VEM-Party updates)
...
do
	...
	local TabFrame1 = CreateFrame("Frame", "VEM_GUI_DropDown", UIParent)
	...
	TabFrame1:Hide()
	TabFrame1:SetParent( VEM_GUI_OptionsFrame )
	...
	for i=1, MAX_BUTTONS, 1 do
		TabFrame1.buttons[i] = CreateFrame("Button", TabFrame1:GetName().."Button"..i, TabFrame1, "VEM_GUI_DropDownMenuButtonTemplate")
		if i == 1 then
	...
	function VEM_GUI:CreateDropdown(title, values, selected, callfunc, width, parent)
		local FrameTitle = "VEM_GUI_DropDown"
		...
		local dropdown = CreateFrame("Frame", FrameTitle..self:GetNewID(), parent or self.frame, "VEM_GUI_DropDownMenuTemplate")
