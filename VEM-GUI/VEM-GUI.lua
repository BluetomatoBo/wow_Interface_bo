...
VEM_GUI = {}
setmetatable(PanelPrototype, {__index = VEM_GUI})
...
function VEM_GUI:ShowHide(forceshow)
	...
	elseif forceshow == false then
		...
		if VEM_GUI_OptionsFrame:IsShown() then
			VEM_GUI_OptionsFrame:Hide()
		else
			self:UpdateModList()
			VEM_GUI_OptionsFrame:Show()
...
do
	VEM_GUI_OptionsFrameTab1:SetText(L.OTabBosses)
	VEM_GUI_OptionsFrameTab2:SetText(L.OTabOptions)
	...
	function VEM_GUI:CreateNewPanel(FrameName, FrameTyp, showsub, sortID)
		...
		panel:SetWidth(VEM_GUI_OptionsFramePanelContainerFOV:GetWidth())
		panel:SetHeight(VEM_GUI_OptionsFramePanelContainerFOV:GetHeight())
		...
		if FrameTyp == "option" or FrameTyp == 2 then
			panel.categoryid = VEM_GUI_Options:CreateCategory(panel, self and self.frame and self.frame.name)
			panel.FrameTyp = 2
		else
			panel.categoryid = VEM_GUI_Bosses:CreateCategory(panel, self and self.frame and self.frame.name)
			panel.FrameTyp = 1
		end
	...
	function PanelPrototype:Destroy()
		if self.frame.FrameTyp == 2 then
			table.remove(VEM_GUI_Options.Buttons, self.frame.categoryid)
		else
			table.remove(VEM_GUI_Bosses.Buttons, self.frame.categoryid)
		end
	...
	function VEM_GUI:GetLastObj()
		return self.lastobject
	end
	function VEM_GUI:SetLastObj(obj)
		self.lastobject = obj
	end
	function VEM_GUI:GetParentsLastObj()
...
do
	local FrameNames = {}
	function VEM_GUI:AddFrame(FrameName)
		table.insert(FrameNames, FrameName)
	end
	function VEM_GUI:IsPresent(FrameName)
...
do
	local framecount = 0
	function VEM_GUI:GetNewID()
		framecount = framecount + 1
		return framecount
	end
	function VEM_GUI:GetCurrentID()
		return framecount
	end
end
...
local function MixinSharedMedia3(mediatype, mediatable)
	...
	-- VEM values (mediatable) first, LibSharedMedia values (sorted alphabetically) afterwards
...
--  arg4 = VEM.Options[arg4]
--  arg5 = VEM.Bars:SetOption(arg5, ...)
--
do
	...
	local function onHyperlinkEnter(self, data, link)
		...
		if linkType ~= "journal" then
			...
			if contentType == "2" then -- EJ section
				...
				GameTooltip:AddLine(name or VEM_CORE_UNKNOWN, 255, 255, 255, 0)
				GameTooltip:AddLine(" ")
				GameTooltip:AddLine(description or VEM_CORE_UNKNOWN, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, 1)
			end
		end
		GameTooltip:Show()
	...
	function PanelPrototype:CreateCheckButton(name, autoplace, textleft, vemvar, dbtvar, soundVal, mod)
		if not name then
			return
		end
		...
		_G[buttonName .. 'Text']:SetText(name or VEM_CORE_UNKNOWN)
		...
		if vemvar and VEM.Options[vemvar] ~= nil then
			button:SetScript("OnShow",  function(self) button:SetChecked(VEM.Options[vemvar]) end)
			button:SetScript("OnClick", function(self) VEM.Options[vemvar] = not VEM.Options[vemvar] end)
		end

		if dbtvar then
			button:SetScript("OnShow",  function(self) button:SetChecked( VEM.Bars:GetOption(dbtvar) ) end)
			button:SetScript("OnClick", function(self) VEM.Bars:SetOption(dbtvar, not VEM.Bars:GetOption(dbtvar)) end)
		end
...
do
	...
	function PanelPrototype:CreateEditBox(text, value, width, height)
		local textbox = CreateFrame('EditBox', FrameTitle..self:GetNewID(), self.frame, 'VEM_GUI_FrameEditBoxTemplate')
...
function PanelPrototype:CreateButton(title, width, height, onclick, FontObject)
	local button = CreateFrame('Button', FrameTitle..self:GetNewID(), self.frame, 'VEM_GUI_OptionsFramePanelButtonTemplate')
	local buttonName = button:GetName()
...
function ListFrameButtonsPrototype:CreateCategory(frame, parent)
	if not type(frame) == "table" then
		VEM:AddMsg("Failed to create category - frame is not a table")
		VEM:AddMsg(debugstack())
		return false
	elseif not frame.name then
		VEM:AddMsg("Failed to create category - frame.name is missing")
		VEM:AddMsg(debugstack())
		return false
	elseif self:IsPresent(frame.name) then
		VEM:AddMsg("Frame ("..frame.name..") already exists")
		VEM:AddMsg(debugstack())
		return false
	end
...
VEM_GUI_Bosses = CreateNewFauxScrollFrameList()
VEM_GUI_Options = CreateNewFauxScrollFrameList()
...
do
	...
	function VEM_GUI_OptionsFrame:UpdateMenuFrame(listframe)
		...
		if listframe:GetParent().tab == 2 then
			TABLE = VEM_GUI_Options:GetVisibleTabs()
		else
			TABLE = VEM_GUI_Bosses:GetVisibleTabs()
		end
		local element
		...
		local selection = VEM_GUI_OptionsFrameBossMods.selection
		if ( selection ) then
			VEM_GUI_OptionsFrame:ClearSelection(listframe, listframe.buttons)
		end

		for i = 1, #buttons do
			element = displayedElements[i + offset]
			if ( not element ) then
				VEM_GUI_OptionsFrame:HideButton(buttons[i])
			else
				VEM_GUI_OptionsFrame:DisplayButton(buttons[i], element)

				if ( selection ) and ( selection == element ) and ( not listframe.selection ) then
					VEM_GUI_OptionsFrame:SelectButton(listframe, buttons[i])
				end
			end
		end
	end
	...
	function VEM_GUI_OptionsFrame:DisplayButton(button, element)
		button:Show()
	...
	function VEM_GUI_OptionsFrame:HideButton(button)
		button:Hide()
	end

	-- This function is for internal use.
	-- Called when a new entry is selected
	function VEM_GUI_OptionsFrame:ClearSelection(listFrame, buttons)
	...
	function VEM_GUI_OptionsFrame:SelectButton(listFrame, button)
	...
	function VEM_GUI_OptionsFrame:CreateButtons(frame)
		...
		local buttons = {}
		local button = CreateFrame("BUTTON", name.."Button1", frame, "VEM_GUI_FrameButtonTemplate")
		...
		for i = 2, maxButtons do
			button = CreateFrame("BUTTON", name.."Button"..i, frame, "VEM_GUI_FrameButtonTemplate")
	...
	function VEM_GUI_OptionsFrame:OnButtonClick(button)
		local parent = button:GetParent()
		local buttons = parent.buttons
		local buttonName = VEM_GUI_OptionsFrame:GetName()

		self:ClearSelection(_G[buttonName.."BossMods"],   _G[buttonName.."BossMods"].buttons)
		self:ClearSelection(_G[buttonName.."VEMOptions"], _G[buttonName.."VEMOptions"].buttons)
		self:SelectButton(parent, button)

		self:DisplayFrame(button.element)
	end

	function VEM_GUI_OptionsFrame:ToggleSubCategories(button)
		local parent = button:GetParent()
...
--			VEM:AddMsg(debugstack())
		...
		if VEM.Options.EnableModels then
			if not modelFrameCreated then
				CreateAnimationFrame()
			end
			VEM_BossPreview.enabled = false
			VEM_BossPreview:Hide()
...
local function CreateOptionsMenu()
	...
	--
	--    yourframe = VEM_GUI_Frame:CreateNewPanel("title", "option")
	--
	--  You can use the VEM widgets by calling methods like
	--
	...
	VEM_GUI_Frame = VEM_GUI:CreateNewPanel(L.TabCategory_Options, "option")
	if VEM.Options.EnableModels then CreateAnimationFrame() end
	do
		----------------------------------------------
		--             General Options              --
		----------------------------------------------
		...
		bmradar:SetScript("OnClick", function(self)
			if VEMRangeCheckRadar and VEMRangeCheckRadar:IsShown() then
				VEMRangeCheckRadar:Hide()
			else
				VEM.RangeCheck:Show(nil, nil, true)
				VEMRangeCheckRadar:Show()
			end
		end)
		...
		VEM.Options.ChallengeBest, function(value)
		...
		VEM.Options.ModelSoundValue, function(value)
			VEM.Options.ModelSoundValue = value
		end
		)
	...
	do
		...
		--            General Warnings           --
		-------------------------------------------
		local generalWarningPanel = VEM_GUI_Frame:CreateNewPanel(L.Tab_GeneralMessages, "option")
...
--		generalCoreArea:CreateCheckButton(L.ShowLoadMessage, true, nil, "ShowLoadMessage")--Only here as a note, this is commented out so inexperienced users don't disable this, but an option for advanced users who want to manually change the value from true to false
	...
	do
		...
		--            Raid Warning Colors            --
		-----------------------------------------------
		local RaidWarningPanel = VEM_GUI_Frame:CreateNewPanel(L.Tab_RaidWarning, "option")
		...
		local RaidWarnSoundDropDown = raidwarnoptions:CreateDropdown(L.RaidWarnSound, Sounds,
			VEM.Options.RaidWarningSound, function(value)
				VEM.Options.RaidWarningSound = value
			end
		)
...
--[[		
		...
		VEM.Options.CountdownVoice2, function(value)
			VEM.Options.CountdownVoice2 = value
			VEM:PlayCountSound(1, VEM.Options.CountdownVoice2)
		end
		)
		...
		local function UpdateColor(self)
			...
			VEM.Options.WarningColors[self.myid].r = r
			VEM.Options.WarningColors[self.myid].g = g
			VEM.Options.WarningColors[self.myid].b = b
		end
		local function ResetColor(id, frame)
			return function(self)
				VEM.Options.WarningColors[id].r = VEM.DefaultOptions.WarningColors[id].r
				VEM.Options.WarningColors[id].g = VEM.DefaultOptions.WarningColors[id].g
				VEM.Options.WarningColors[id].b = VEM.DefaultOptions.WarningColors[id].b
				frame:SetColorRGB(VEM.Options.WarningColors[id].r, VEM.Options.WarningColors[id].g, VEM.Options.WarningColors[id].b)
			end
		end
		local function UpdateColorFrames(color, text, rset, id)
			...
			color:SetColorRGB(VEM.Options.WarningColors[id].r, VEM.Options.WarningColors[id].g, VEM.Options.WarningColors[id].b)
		...
		do
			...
			movemebutton:SetScript("OnClick", function(self)
				VEM:Schedule(20, hideme)
				VEM.Bars:CreateBar(20, L.BarWhileMove)
				if not anchorFrame then
					anchorFrame = CreateFrame("Frame", "VEM_GUI_RaidWarnAnchor", UIParent)
					...
					anchorFrame.texture:SetTexture("Interface\\Addons\\VEM-GUI\\textures\\dot.blp")
					...
					anchorFrame:SetScript("OnMouseUp", function(self)
						...
						VEM.Options.RaidWarningPosition.Y = yOfs
						VEM:Schedule(15, hideme)
						VEM.Bars:CreateBar(15, L.BarWhileMove)
					end)
					do
						local elapsed = 10
	...
	do
		...
		movemebutton:SetNormalFontObject(GameFontNormalSmall)
		movemebutton:SetHighlightFontObject(GameFontNormalSmall)
		movemebutton:SetScript("OnClick", function() VEM.Bars:ShowMovableBar() end)
		...
		color1reset:SetScript("OnClick", function(self)
			color1:SetColorRGB(VEM.Bars:GetDefaultOption("StartColorR"), VEM.Bars:GetDefaultOption("StartColorG"), VEM.Bars:GetDefaultOption("StartColorB"))
		end)
		color2reset:SetScript("OnClick", function(self)
			color2:SetColorRGB(VEM.Bars:GetDefaultOption("EndColorR"), VEM.Bars:GetDefaultOption("EndColorG"), VEM.Bars:GetDefaultOption("EndColorB"))
		end)
		...
		color1:SetScript("OnShow", function(self) self:SetColorRGB(
								VEM.Bars:GetOption("StartColorR"),
								VEM.Bars:GetOption("StartColorG"),
								VEM.Bars:GetOption("StartColorB"))
		...
		color2:SetScript("OnShow", function(self) self:SetColorRGB(
								VEM.Bars:GetOption("EndColorR"),
								VEM.Bars:GetOption("EndColorG"),
								VEM.Bars:GetOption("EndColorB"))
		...
		color1:SetScript("OnColorSelect", function(self)
							VEM.Bars:SetOption("StartColorR", select(1, self:GetColorRGB()))
							VEM.Bars:SetOption("StartColorG", select(2, self:GetColorRGB()))
							VEM.Bars:SetOption("StartColorB", select(3, self:GetColorRGB()))
							color1text:SetTextColor(self:GetColorRGB())
						  end)
		color2:SetScript("OnColorSelect", function(self)
							VEM.Bars:SetOption("EndColorR", select(1, self:GetColorRGB()))
							VEM.Bars:SetOption("EndColorG", select(2, self:GetColorRGB()))
							VEM.Bars:SetOption("EndColorB", select(3, self:GetColorRGB()))
							color2text:SetTextColor(self:GetColorRGB())
						  end)

		local Textures = MixinSharedMedia3("statusbar", {
			{	text	= "Default",	value 	= "Interface\\AddOns\\VEM-Core\\textures\\default.tga", 	texture	= "Interface\\AddOns\\VEM-Core\\textures\\default.tga"	},
			...
			{	text	= "Glaze",	value 	= "Interface\\AddOns\\VEM-Core\\textures\\glaze.tga", 		texture	= "Interface\\AddOns\\VEM-Core\\textures\\glaze.tga"	},
			{	text	= "Otravi",	value 	= "Interface\\AddOns\\VEM-Core\\textures\\otravi.tga", 		texture	= "Interface\\AddOns\\VEM-Core\\textures\\otravi.tga"	},
			{	text	= "Smooth",	value 	= "Interface\\AddOns\\VEM-Core\\textures\\smooth.tga", 		texture	= "Interface\\AddOns\\VEM-Core\\textures\\smooth.tga"	}
		})

		local TextureDropDown = BarSetup:CreateDropdown(L.BarTexture, Textures,
			VEM.Bars:GetOption("Texture"), function(value)
		...
		local FontDropDown = BarSetup:CreateDropdown(L.Bar_Font, Fonts, VEM.Bars:GetOption("Font"),
			function(value)
		...
		local smalldummybar = VEM.Bars:CreateDummyBar()
		...
		local hugedummybar = VEM.Bars:CreateDummyBar()
	...
	do
		...
		local FontDropDown = specArea:CreateDropdown(L.SpecWarn_FontType, Fonts, VEM.Options.SpecialWarningFont,
			function(value)
		...
		local SpecialWarnSoundDropDown = specArea:CreateDropdown(L.SpecialWarnSound, Sounds,
			VEM.Options.SpecialWarningSound, function(value)
	...
	do
		...
		BarWidthSlider:SetScript("OnShow", function(self) self:SetValue(VEM.Options.HealthFrameWidth or 100) end)
		BarWidthSlider:HookScript("OnValueChanged", function(self)
				VEM.Options.HealthFrameWidth = self:GetValue()
				VEM.BossHealth:UpdateSettings()
		end)
		...
		showbutton:SetScript("OnClick", function()
				VEM.BossHealth:Show("Health Frame")
				VEM.BossHealth:AddBoss(createDummyFunc(25), "TestBoss 1")
				VEM.BossHealth:AddBoss(createDummyFunc(50), "TestBoss 2")
				VEM.BossHealth:AddBoss(createDummyFunc(75), "TestBoss 3")
				VEM.BossHealth:AddBoss(createDummyFunc(100), "TestBoss 4")
		end)
	end
	...
	do
		...
		VEM.Options.MovieFilter, function(value)
			VEM.Options.MovieFilter = value
		end
		)
	...
	if L.TranslationBy then
		VEM_GUI_OptionsFrameTranslation:SetText(L.TranslationByPrefix .. L.TranslationBy)
	end
	VEM_GUI_OptionsFrameWebsite:SetText(L.Website)
...
do
	...
	local function LoadAddOn_Button(self)
		if VEM:LoadMod(self.modid) then
			...
			VEM_GUI_OptionsFrameBossMods:Hide()
			VEM_GUI_OptionsFrameBossMods:Show()
		end
	end
		...
		for k,addon in ipairs(VEM.AddOns) do
			if not Categories[addon.category] then
				...
				elseif expLevel == 2 then
					Categories[addon.category] = VEM_GUI:CreateNewPanel(L["TabCategory_"..addon.category:upper()] or L.TabCategory_Other, nil, (addon.category:upper()=="WotLK"))
				elseif expLevel == 1 then
					Categories[addon.category] = VEM_GUI:CreateNewPanel(L["TabCategory_"..addon.category:upper()] or L.TabCategory_Other, nil, (addon.category:upper()=="BC"))
				end
	...
	function VEM_GUI:CreateBossModPanel(mod)
		if not mod.panel then
			VEM:AddMsg("Couldn't create boss mod panel for "..mod.localization.general.name)
			return false
		end
		local panel = mod.panel
		local category

		local iconstat = panel.frame:CreateFontString("VEM_GUI_Mod_Icons"..mod.localization.general.name, "ARTWORK")
