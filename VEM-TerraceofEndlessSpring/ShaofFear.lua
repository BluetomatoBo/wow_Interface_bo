local mod	= VEM:NewMod(709, "VEM-TerraceofEndlessSpring", nil, 320)
...
local function spectimestart()
	if huddle == 1 and spout == 1 and strike == 0 then
		sndWOP:Schedule(5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_ylxjzb.mp3")
	...
	if huddle == 1 and spout == 0 and strike == 0 then
		sndWOP:Schedule(6, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\specialsoon.mp3")
		timerSpoStrCD:Start()
	end
	if huddle == 0 and spout == 1 and strike == 0 then
		timerHuddleInTerrorCD:Start()
		specWarningpreHud:Schedule(8)
		sndWOP:Schedule(6, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_wszb.mp3")
		if not mod:IsTank() then
			sndWOP:Schedule(7, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\holdit.mp3")
		end
	end
...
function mod:LeavePlatform()
	if onPlatform then
		...
		if VEM.BossHealth:IsShown() then
			VEM.BossHealth:RemoveBoss(61038)
			VEM.BossHealth:RemoveBoss(61042)
			VEM.BossHealth:RemoveBoss(61046)
		end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(129147) then
		...
		if args:IsPlayer() then
			...
			if self:AntiSpam(2, 4) then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\telesoon.mp3")--準備傳送
			end
	...
	elseif args:IsSpellID(132007) then
		if not mod:IsDps() then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_llj.mp3")--六連擊
		end
		ThrashCount = 0
		swingcount = -1
		timerThrashCD:Start()
	...
	elseif args:IsSpellID(120047) and MobID and MobID == args:GetSrcCreatureID() then--might change
		...
		if not self.Options.pscount then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_kbpszb.mp3")--恐怖噴散
		end
	...
	elseif args:IsSpellID(131996) and not onPlatform then
		...
		specWarnThrash:Show()
		if not mod:IsDps() then
			sndDD:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\doubleat.mp3")--雙重攻擊	
			timerThrashCD:Start()
			if phase == 2 then
				...
				if inRange then
					self:Schedule(0.3, function()
						if self:AntiSpam(5, 20) then
							...
							sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\keepmove.mp3")
						end
					end)
 				end
 			end
	...
	elseif args:IsSpellID(120669) then--赤裸
		sndWOP:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.mp3")
		...
		if mod:IsTank() then
			if timeryinmo:GetTime() < 20 then
				sndWOP:Schedule(26, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.mp3") --赤裸準備
				sndWOP:Schedule(27.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
				sndWOP:Schedule(28.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
				sndWOP:Schedule(29.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			end
		end
	elseif args:IsSpellID(120629) then
		...
		if self:AntiSpam(5, 6) then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_wsks.mp3")
			specWarnweisuo:Show(kongjuCount)
			timerweisuo:Start(50, kongjuCount + 1)
			huddle = 1
			spectimestart()
			if kongjuCount % 4 == 1 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			elseif kongjuCount % 4 == 2 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			elseif kongjuCount % 4 == 3 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			elseif kongjuCount % 4 == 0 then
				sndWOPWSCOUNT:Schedule(1, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			end
			if MyJS() then
...
function mod:SPELL_CAST_START(args)
	...
	elseif args:IsSpellID(119862) and playkbpsound and not platformGUIDs[args.sourceGUID] then--Best way to track engaging one of the side adds, they cast this instantly.
		...
		if VEM.BossHealth:IsShown() then
			VEM.BossHealth:AddBoss(MobID, args.sourceName)
		end
	elseif args:IsSpellID(119888) and MobID and MobID == args:GetSrcCreatureID() then
	...
	elseif args:IsSpellID(120519) then
		specWarnshuipo:Show()
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_spzb.mp3") --水魄準備
	elseif args:IsSpellID(120455) then --隐没
		...
		sndWOP:Schedule(9, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\specialsoon.mp3") --準備特別技能
	...
	elseif args:IsSpellID(120458) then --浮現
		if yinmoCount == 1 then
			...
			if mod:IsTank() then
				sndWOP:Schedule(10, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.mp3") --赤裸準備
				sndWOP:Schedule(11.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
				sndWOP:Schedule(12.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
				sndWOP:Schedule(13.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			end
		else
			if (timerNakedAndAfraidCD:GetTime() > 15) or (timerNakedAndAfraidCD:GetTime() == 0) then
				timerNakedAndAfraidCD:Update(15, 30)
				if mod:IsTank() then
					sndWOP:Schedule(11, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_clzb.mp3") --赤裸準備
					sndWOP:Schedule(12.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
					sndWOP:Schedule(13.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
					sndWOP:Schedule(14.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
				end
			end
		end
		...
		if mod.Options.InfoFrame then
			VEM.InfoFrame:SetHeader(EJ_GetSectionInfo(6107).."["..kjzznow.."-"..infowjzz.."]: "..kjzz)
		end
	elseif args:IsSpellID(120672) then
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\shockwave.mp3") --震懾波
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(119983) then
		...
		if self.Options.pscount then
			if kbpscount == 1 then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3") --恐怖噴散計數
			elseif (kbpscount == 2) or (kbpscount == 5) or (kbpscount == 6) or (kbpscount == 10) or (kbpscount == 14) then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			elseif (kbpscount == 3) or (kbpscount == 7) or (kbpscount == 9) or (kbpscount == 11) or (kbpscount == 15) then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			elseif (kbpscount == 4) or (kbpscount == 8) or (kbpscount == 12) or (kbpscount == 13) or (kbpscount == 16) then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			end
		end
		if MobID == 61046 then
			if (VEM.ShaAssistStarModeChosed == "Dps" and kbpscount == 1) or (VEM.ShaAssistStarModeChosed == "Healther" and kbpscount == 6) then
				MoveWarningLeft:Show() --左
				if not self.Options.pscount then
					sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\moveleft.mp3")
				end
			end
		end	
		if MobID == 61042 then
			if (VEM.ShaAssistStarModeChosed == "Dps" and kbpscount == 1) or (VEM.ShaAssistStarModeChosed == "Healther" and kbpscount == 7) then
				MoveWarningRight:Show()  --右
				if not self.Options.pscount then
					sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\moveright.mp3")
				end
			end
		end	
		if MobID == 61038 then
			if (VEM.ShaAssistStarModeChosed == "Dps" and kbpscount == 4) or (VEM.ShaAssistStarModeChosed == "Healther" and kbpscount == 4) then
				MoveWarningRight:Show()  --右
				if not self.Options.pscount then
					sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\moveright.mp3")
				end
		...
		if kbpscount == 16 then
			if VEM.ShaAssistStarModeChosed == "Dps" or VEM.ShaAssistStarModeChosed == "Healther" then
				MoveWarningBack:Show()
				if not self.Options.pscount then
					sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\end.mp3")
				end
			end
		end
	end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 119108 and self:AntiSpam(2, 3) then
		if not onPlatform then
			...
			if mod:IsDps() then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_kdkjzz.mp3") --快打恐懼之子
			end
		end
	elseif spellId == 114936 and self:AntiSpam(5, 6) then
		...
		if mod.Options.InfoFrame and (not mod.Options.InfoFrameTankMode) then
			VEM.InfoFrame:SetHeader(EJ_GetSectionInfo(6107))
			VEM.InfoFrame:Show(10, "playerbaddebuff", 120629)
		end
		if VEM.BossHealth:IsShown() then
			VEM.BossHealth:RemoveBoss(61038)
			VEM.BossHealth:RemoveBoss(61042)
		...
		sndWOP:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_tenkj.mp3")
		sndWOP:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
...
function mod:OnSync(msg)
	if msg == "preptwo" then
		if not prewarnedPhase2 then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ptwo.mp3")
			prewarnedPhase2 = true
			warnPhase2Soon:Show()
		end
	end
...
function mod:SWING_DAMAGE(sourceGUID)
	...
	if cid == 60999 and self:AntiSpam(1, 11) then
		...
		if mod.Options.InfoFrameTankMode then
			if phase == 1 then
				VEM.InfoFrame:SetHeader(GetSpellInfo(135088))
			end
			if phase == 2 then
				if swingcount == 0 then
					if ThrashCount == 0 then
						VEM.InfoFrame:Show(1, "other", "", GetSpellInfo(7389)..": "..L.liulian)
					else
						VEM.InfoFrame:Show(1, "other", L.sanlian..": "..ThrashCount, GetSpellInfo(7389)..": "..L.sanlian)
					end
				else
					VEM.InfoFrame:Show(1, "other", L.sanlian..": "..ThrashCount, GetSpellInfo(7389)..": "..swingcount)
				end
			else
				if swingcount == 0 then
					VEM.InfoFrame:Show(1, "other", "", GetSpellInfo(7389)..": "..L.sanlian)
				else
					VEM.InfoFrame:Show(1, "other", "", GetSpellInfo(7389)..": "..swingcount)
				end
			end
		end
	end
