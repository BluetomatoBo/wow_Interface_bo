local mod	= VEM:NewMod(729, "VEM-TerraceofEndlessSpring", nil, 320)
...
function mod:OnCombatStart(delay)
	if self.Options.RangeFrame then
		VEM.RangeCheck:Show(3, bossTank)
	end
	hideDebug = 0
	damageDebug = 0
	timeDebug = 0
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(123461) then
		...
		if mod:IsHealer() then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_slkd.mp3") --首領快打
		end
	...
	elseif args:IsSpellID(123121) then
		if not mod:IsTank() and args:IsPlayer() and (not hideActive) and self:AntiSpam(2, 1) then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\runaway.mp3") --快躲开
		end
	...
	elseif args:IsSpellID(123705) then
		...
		if self.Options.HudMAP then
			...
			if not MWMarkers[args.destName] then
				MWMarkers[args.destName] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("timer", args.destName, 5, nil, 1, 1, 1, 0.5):RegisterForAlerts())
			end
		end
	end
end
...
mod:RegisterOnUpdateHandler(function(self)
	if self.Options.SetIconOnGuardfix and guardActivated > 0 and VEM:GetRaidRank() > 0 then
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(123244) then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_yszb.mp3") --隱身準備
		sndWOP:Schedule(1, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\scattersoon.mp3")--注意分散
		...
		if self.Options.RangeFrame then
			VEM.RangeCheck:Show(3)--Show everyone during hide
		end
	elseif args:IsSpellID(123705) then
		self:ScaryFogRepeat()
	end
...
function mod:INSTANCE_ENCOUNTER_ENGAGE_UNIT(event)
	...
	sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_mop_ysjs.mp3") --隱身結束
	if self.Options.RangeFrame then
