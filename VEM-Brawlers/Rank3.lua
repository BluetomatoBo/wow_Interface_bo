local mod	= VEM:NewMod("BrawlRank3", "VEM-Brawlers")
...
local brawlersMod = VEM:GetModByName("Brawlers")
local blatGUID = 0
...
function mod:SPELL_CAST_START(args)
	...
	elseif args.spellId == 132666 then
		...
		if brawlersMod:PlayerFighting() then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\firewall.mp3")
		end
...
function mod:UNIT_TARGET()
	if self.Options.SetIconOnBlat and not VEM.Options.DontSetIcons and UnitGUID("target") == blatGUID then
