local mod	= VEM:NewMod("Brawlers", "VEM-Brawlers")
...
local currentZoneID = VEM:GetCurrentArea()--As core what current area is on load, since core should know
...
function mod:ZONE_CHANGED_NEW_AREA()
	currentZoneID = VEM:GetCurrentArea()
	if currentZoneID == 369 or currentZoneID == 1043 then
		modsStopped = false
		eventsRegistered = true
	...
	for i = 1, 9 do
		local mod2 = VEM:GetModByName("BrawlRank" .. i)
		if mod2 then
			mod2:Stop()--Stop all timers and warnings
		end
	end
	for i = 1, 2 do
		local mod2 = VEM:GetModByName("BrawlRare" .. i)
		if mod2 then
			mod2:Stop()--Stop all timers and warnings
		end
	end
...
function mod:UNIT_AURA(uId)
	...
	if currentQueueRank and currentQueueRank ~= lastRank then
		...
		if self.Options.SpeakOutQueue then
			VEM:PlayCountSound(currentQueueRank)
		end
	end
end
