...
local hordeTowerIcon = "Interface\\AddOns\\VEM-PvP\\Textures\\OrcTower"
	...
	if gateHP[guid] == nil then -- first hit
		...
		if self.Options.ShowGatesHealth then
			if not VEM.BossHealth:IsShown() then
				VEM.BossHealth:Show(L.GatesHealthFrame)
			end
			VEM.BossHealth:AddBoss(function() return gateHP[guid]/6000	end, destName)
		end
	end
