...
function mod:ZONE_CHANGED_NEW_AREA()
	if select(2, IsInInstance()) == "pvp" then
		-- hardcoded version sync as VEM only syncs if you join a raid and you technically don't join a new raid if you enter a battleground while you are already in a raid group
		SendAddonMessage("H", "", "INSTANCE_CHAT")
		self:Schedule(3, VEM.RequestTimers, VEM)
		inviteTimer:Stop()
		SetMapToCurrentZone() -- for GetMapLandmarkInfo()
		if self.Options.HideBossEmoteFrame then
			VEM:ToggleRaidBossEmoteFrame(1, true)
		end
	else
		if self.Options.HideBossEmoteFrame then
			VEM:ToggleRaidBossEmoteFrame(0, true)
		end
	end
	for i, v in ipairs(VEM:GetModByName("z30").timers) do v:Stop() end
	for i, v in ipairs(VEM:GetModByName("z489").timers) do v:Stop() end
	for i, v in ipairs(VEM:GetModByName("z529").timers) do v:Stop() end
	for i, v in ipairs(VEM:GetModByName("z566").timers) do v:Stop() end
	for i, v in ipairs(VEM:GetModByName("z628").timers) do v:Stop() end
	for i, v in ipairs(VEM:GetModByName("z726").timers) do v:Stop() end
	for i, v in ipairs(VEM:GetModByName("z727").timers) do v:Stop() end
	for i, v in ipairs(VEM:GetModByName("z761").timers) do v:Stop() end
	for i, v in ipairs(VEM:GetModByName("z998").timers) do v:Stop() end
	VEM:GetModByName("z30"):Unschedule()
	VEM:GetModByName("z489"):Unschedule()
	VEM:GetModByName("z529"):Unschedule()
	VEM:GetModByName("z566"):Unschedule()
	VEM:GetModByName("z628"):Unschedule()
	VEM:GetModByName("z726"):Unschedule()
	VEM:GetModByName("z727"):Unschedule()
	VEM:GetModByName("z761"):Unschedule()
	VEM:GetModByName("z998"):Unschedule()
end
