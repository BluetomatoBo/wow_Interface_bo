...
options = {
	...
	Texture = {
		type = "string",
		default = "Interface\\AddOns\\VEM-DefaultSkin\\textures\\default.tga",
	},
	StartColorR = {
		type = "number",
		default = 1,
...
do
	...
	function DBT:RegisterSkin(id)
		if id:sub(0, 4) == "VEM-" then
			id = id:sub(5)
		end
		local obj = skins[id]
		if not obj then
			error("unknown skin id; the id must be equal to the addon's name (with the VEM- prefix being optional)", 2)
		end
		obj.loaded = true
		obj.defaults = {}
		return obj
	...
	for i = 1, GetNumAddOns() do
		if GetAddOnMetadata(i, "X-VEM-Timer-Skin") then
			...
			local name = GetAddOnMetadata(i, "X-VEM-Timer-Skin-Name")
			skins[id] = setmetatable({
				name = name
			}, skin)

			-- load textures and fonts that might be embedded in this skin (to make them available to other skins)
			local skinTextures = { strsplit(",", GetAddOnMetadata(i, "X-VEM-Timer-Skin-Textures") or "") }
			local skinTextureNames = { strsplit(",", GetAddOnMetadata(i, "X-VEM-Timer-Skin-Texture-Names") or "") }
			...
			local skinFonts = { strsplit(",", GetAddOnMetadata(i, "X-VEM-Timer-Skin-Fonts") or "") }
			local skinFontNames = { strsplit(",", GetAddOnMetadata(i, "X-VEM-Timer-Skin-Font-Names") or "") }
