...
--  Locals --
-------------
local bossHealth = VEM.BossHealth
local bars = {}
local barCache = {}
local updateFrame
local getBarId
...
menu = {
	{
		text = LOCK_FRAME,
		checked = false, -- requires VEM.Options which is not available yet
		func = function()
			menu[1].checked = not menu[1].checked
			VEM.Options.HealthFrameLocked = menu[1].checked
		end
	},
	{
		text = VEM_CORE_BOSSHEALTH_HIDE_FRAME,
		notCheckable = true,
...
local function onMouseDown(self, button)
	if button == "LeftButton" and not VEM.Options.HealthFrameLocked then
		anchor.moving = true
		anchor:StartMoving()
	end
end
...
local function onMouseUp(self, button)
	...
	VEM.Options.HPFramePoint = point
	VEM.Options.HPFrameX = x
	VEM.Options.HPFrameY = y
...
local function updateBarStyle(bar, id)
	bar:ClearAllPoints()
	if VEM.Options.HealthFrameGrowUp then
	...
	local width = VEM.Options.HealthFrameWidth
	if width < 175 then -- these health frames really suck :(
		barbar:ClearAllPoints()
		barbar:SetPoint("CENTER", barbar:GetParent(), "CENTER", -6, 0)
		bar:SetWidth(VEM.Options.HealthFrameWidth)
		barborder:SetWidth(VEM.Options.HealthFrameWidth * 0.99)
		barbar:SetWidth(VEM.Options.HealthFrameWidth * 0.95)
	elseif width >= 225 then
		barbar:ClearAllPoints()
		barbar:SetPoint("CENTER", barbar:GetParent(), "CENTER", 5, 0)
		bar:SetWidth(VEM.Options.HealthFrameWidth)
		barborder:SetWidth(VEM.Options.HealthFrameWidth * 0.995)
		barbar:SetWidth(VEM.Options.HealthFrameWidth * 0.965)
	else
		bar:SetWidth(VEM.Options.HealthFrameWidth)
		barborder:SetWidth(VEM.Options.HealthFrameWidth * 0.99)
		barbar:SetWidth(VEM.Options.HealthFrameWidth * 0.95)
...
local function createFrame(self)
	...
	anchor:SetPoint(VEM.Options.HPFramePoint, UIParent, VEM.Options.HPFramePoint, VEM.Options.HPFrameX, VEM.Options.HPFrameY)
	...
	dropdownFrame = CreateFrame("Frame", "VEMBossHealthDropdown", anchor, "UIDropDownMenuTemplate")
	menu[1].checked = VEM.Options.HealthFrameLocked
end
...
local function createBar(self, name, ...) -- the vararg will also contain the name, see method AddBoss for details (TODO: this should be handled earlier, seriously...)
	local bar = table.remove(barCache, #barCache) or CreateFrame("Frame", "VEM_BossHealth_Bar_"..getBarId(), anchor, "VEMBossHealthBarTemplate")
	bar:Show()
...
--	end
	...
	function updateFrame(self, e)
		...
		if t >= 0.5 then
			t = t - 0.5
--			if #bars > VEM.Options.HPFrameMaxEntries then
--				sortingEnabled = true
--			end
...
--				if i > VEM.Options.HPFrameMaxEntries then
--					v:Hide()
--				else
--					v:Show()
--				end
