VEMHudMap = LibStub("AceAddon-3.0"):NewAddon("VEMHudMap", "AceEvent-3.0")
local mod = VEMHudMap
...
local textureLookup = {
	...
	highlight = [[Interface\AddOns\VEM-Core\textures\alert_circle]],
	timer			= [[Interface\AddOns\VEM-Core\textures\timer]],
...
function mod:OnInitialize()
	self.canvas = CreateFrame("Frame", "VEMHudMapCanvas", UIParent)
...
Edge = setmetatable({
	...
	New = function(self, r, g, b, a, srcPlayer, dstPlayer, sx, sy, dx, dy, lifetime)
		...
		if not t then
			...
			local line = "Interface\\AddOns\\VEM-Core\\textures\\line"
...
function mod:GetMinimapSize()
	...
	-- return minimapSize[indoors and "indoor" or "outdoor"][VEMHudMapMinimap:GetZoom()]
end
