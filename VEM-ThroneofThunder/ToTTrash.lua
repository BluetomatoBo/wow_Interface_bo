local mod	= VEM:NewMod("ToTTrash", "VEM-ThroneofThunder")
...
local function SpiritFireTarget(sGUID)
	...
	for uId in VEM:GetGroupMembers() do
		if UnitGUID(uId.."target") == sGUID then
			targetname = VEM:GetUnitFullName(uId.."targettarget")
			break
		end
	end
...
function mod:SPELL_CAST_START(args)
	...
	if args.spellId == 139895 then
		...
		if self.Options.RangeFrame and not VEM.RangeCheck:IsShown() then
	...
	elseif args.spellId == 139899 then
		...
		if mod:IsMelee() then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\justrun.mp3")
		end
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	if args.spellId == 139322 then--Or 139559, not sure which
		...
		if args:IsPlayer() then
			...
			if self.Options.HudMAP then
				lightmaker[args.destName] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("timer", args.destName, 10, 10, 1, 1, 1, 0.7):RegisterForAlerts())
			end
		end
		if self.Options.RangeFrame and not VEM.RangeCheck:IsShown() then
	...
	elseif args.spellId == 140296 then
		...
		if args.sourceGUID == UnitGUID("target") then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\stopattack.mp3")
		end
	end
end
