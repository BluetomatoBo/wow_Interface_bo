local mod	= VEM:NewMod(829, "VEM-ThroneofThunder", nil, 362)
...
local function intTTtime()
	...
	if XN1 > 20 then
		sndWOP:Schedule(XN1-20, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_xnzb.mp3")
	end
	if QL1 > 20 then
		sndWOP:Schedule(QL1-20, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_qlzb.mp3")
	end
	if BH1 > 20 then
		sndWOP:Schedule(BH1-20, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_bhzb.mp3")
	end
	if HH1 > 20 then
		sndWOP:Schedule(HH1-20, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_hhttzb.mp3")
	end
	if XN2 > 20 then
		sndWOP:Schedule(XN2-20, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_xnzb.mp3")
	end
	if QL2 > 20 then
		sndWOP:Schedule(QL2-20, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_qlzb.mp3")
	end
	if BH2 > 20 then
		sndWOP:Schedule(BH2-20, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_bhzb.mp3")
	end
	if HH2 > 20 then
		sndWOP:Schedule(HH2-20, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_hhttzb.mp3")
	end
...
local function showTTtime(nextTT)
	if TTA[sortedTTA[nextTT]] and TTA[sortedTTA[nextTT]] > 0 then
		if mod.Options.InfoFrame then
			VEM.InfoFrame:Hide()
			VEM.InfoFrame:SetHeader(L.NextTT)
			VEM.InfoFrame:Show(1, "time", sortedTTA[nextTT], TTA[sortedTTA[nextTT]]- (GetTime()-combatstarttime))
		end
	end
end
...
function mod:OnCombatStart(delay)
	...
	if self.Options.RangeFrame then
		VEM.RangeCheck:Show(8, not constellationRunner)
	end	
	intTTtime()	
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args.spellId == 137375 then
		...
		if args:IsPlayer() or mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_mys.mp3") --夢魘獸出現
		else
	...
	elseif args.spellId == 138264 and args:IsPlayer() then  --白虎
		if self.Options.HudMAP then
			self:Schedule(1, function()
				lightmaker["A1"] = register(VEMHudMap:AddEdge(0, 1, 0, 1, nil, nil, nil,705,352,731,381))
				lightmaker["A2"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,731,381,709,377))
				lightmaker["A3"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,709,377,686,373))
				lightmaker["A4"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,686,373,712,398))
				lightmaker["A5"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,712,398,709,376))
				lightmaker["A6"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,709,376,705,354))
			end)
		end
	elseif args.spellId == 138267 and args:IsPlayer() then  --青龍
		if self.Options.HudMAP then
			self:Schedule(1, function()
				lightmaker["B1"] = register(VEMHudMap:AddEdge(0, 1, 0, 1, nil, nil, nil,693,393,686,371))
				lightmaker["B2"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,686,371,710,377))
				lightmaker["B3"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,710,377,704,353))
				lightmaker["B4"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,704,353,725,361))
			end)
		end
	elseif args.spellId == 138254 and args:IsPlayer() then  --玄牛
		if self.Options.HudMAP then
			self:Schedule(1, function()
				lightmaker["C1"] = register(VEMHudMap:AddEdge(0, 1, 0, 1, nil, nil, nil,713,401,709,376))
				lightmaker["C2"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,709,376,705,352))
				lightmaker["C3"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,705,352,688,370))
				lightmaker["C4"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,688,370,709,376))
				lightmaker["C5"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,709,376,732,383))
			end)
		end
	elseif args.spellId == 138189 and args:IsPlayer() then  --红鹤
		if self.Options.HudMAP then
			self:Schedule(1, function()
				lightmaker["D1"] = register(VEMHudMap:AddEdge(0, 1, 0, 1, nil, nil, nil,733,382,705,353))
				lightmaker["D2"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,705,353,687,371))
				lightmaker["D3"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,687,371,709,377))
				lightmaker["D4"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,709,377,732,383))
				lightmaker["D5"] = register(VEMHudMap:AddEdge(1, 1, 1, 1, nil, nil, nil,732,383,713,401))
			end)
		end		
	elseif args.spellId == 138300 and self:AntiSpam(40, 10) then
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_xntt.mp3")--玄牛圖騰
		specWarnTT1:Show()
		TTstart = TTstart + 1
		showTTtime(TTstart+1)
	elseif args.spellId == 138855 and self:AntiSpam(40, 11) then
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_bhtt.mp3")--白虎圖騰
		specWarnTT2:Show()
		TTstart = TTstart + 1
		showTTtime(TTstart+1)
	elseif args.spellId == 138306 and self:AntiSpam(40, 12) then
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_yjtt.mp3")--玉蛟圖騰
		specWarnTT3:Show()
		TTstart = TTstart + 1
		showTTtime(TTstart+1)
	end
...
function mod:UNIT_DIED(args)
	...
	if cid == 68905 then--Lu'lin
		...
		warnDay:Show()
		sndWOPCX:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\defensive.mp3")
		sndWOPCX:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_cxzb.mp3")
		sndWOPCX:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
		sndWOPCX:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
		sndWOPCX:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		sndWOPCX:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 137105 then--Suen Ports away (Night Phase)
		...
		--timerFlamesOfPassionCD:Cancel()
		warnNight:Show()
		sndWOP:Schedule(180, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\dayphase.mp3")--白天準備
		sndWOP:Schedule(181, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
		sndWOP:Schedule(182, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		sndWOP:Schedule(183, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		if Sunlive then
			timerDayCD:Start()
			timerDuskCD:Start()
...
function mod:OnSync(msg)
	if msg == "Phase2" and not phase2Started then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_bzjd.mp3")--白晝開始
	...
	elseif msg == "Phase3Yell" and not phase3Started then -- Split from phase3 sync to prevent who running older version not to show bad timers.
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_hhzb.mp3")--黃昏準備
		sndWOP:Schedule(1, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
		sndWOP:Schedule(2, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
		sndWOP:Schedule(3, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
		sndWOP:Schedule(4, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		sndWOP:Schedule(5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		sndWOP:Schedule(6, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_hhjd.mp3") --黃昏開始
		sndWOPCX:Schedule(25, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_cxzb.mp3")
		sndWOPCX:Schedule(26, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
		sndWOPCX:Schedule(27, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
		sndWOPCX:Schedule(28, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		sndWOPCX:Schedule(29, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
	...
	elseif msg == "Comet" then
		if self:AntiSpam(10, 5) then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_hbcx.mp3") --寒冰出現
	...
	elseif msg == "TidalForce" then
		if self:AntiSpam(10, 6) then
			...
			sndWOPCX:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			if MyJSE() then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\defensive.mp3")
			else
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_cxzl.mp3") --潮汐之力
			end
			sndWOPCX:Schedule(68, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_cxzb.mp3")
			sndWOPCX:Schedule(69, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			sndWOPCX:Schedule(70, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndWOPCX:Schedule(71, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			sndWOPCX:Schedule(72, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			...
			if phase3Started then
				...
				if MyJSB() then
					...
					if mod:IsRanged() then
						...
						sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_xzzb.mp3")--星宙准备
					end
				end
				sndYX:Schedule(1.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
				sndYX:Schedule(2.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
				sndYX:Schedule(3.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
				sndYX:Schedule(4.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			else
				...
				elseif mod:IsRanged() then
					sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\scattersoon.mp3")
					sndYX:Schedule(1.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
					sndYX:Schedule(2.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
					sndYX:Schedule(3.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
					sndYX:Schedule(4.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
	...
	elseif msg == "Inferno" then
		if self:AntiSpam(10, 8) then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_hzly.mp3") --核子煉獄
			if phase3Started then
				timerNuclearInfernoCD:Start(73, NuclearInfernoCount + 1)
				if MyJSD() then
					sndWOP:Schedule(2, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\defensive.mp3")
				end
			else
			...
			sndWOP:Schedule(11, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\scatter.mp3")
		end
	elseif msg == "TearsOfSun" then
		if self:AntiSpam(10, 9) then
			warnTearsOfSun:Show()
			specWarnTearsOfSun:Show()
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_lrzl.mp3")
