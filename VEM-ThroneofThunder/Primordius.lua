local mod	= VEM:NewMod(820, "VEM-ThroneofThunder", nil, 362)
...
local function showspellinfo()
	if mod.Options.InfoFrame then
		...
		if #sh == 1 then
			VEM.InfoFrame:Show(1, "other", sh[1].V, sh[1].K)
		elseif #sh == 2 then
			VEM.InfoFrame:Show(2, "other", sh[1].V, sh[1].K, sh[2].V, sh[2].K)
		elseif #sh == 3 then
			VEM.InfoFrame:Show(3, "other", sh[1].V, sh[1].K, sh[2].V, sh[2].K, sh[3].V, sh[3].K)
		elseif #sh == 4 then
			VEM.InfoFrame:Show(4, "other", sh[1].V, sh[1].K, sh[2].V, sh[2].K, sh[3].V, sh[3].K, sh[4].V, sh[4].K)
		end
	end
end
...
--	print("VEM Debug Spawn: ", bigOozeAlive)
	warnViscousHorror:Show(bigOozeCount)
	specWarnViscousHorror:Show(bigOozeCount)
	if mod:IsTank() then
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_drn.mp3")--大軟泥
	end
	...
	if VEM:GetRaidRank() > 0 and self.Options.SetIconOnBigOozes then--Only register event if option is turned on, otherwise no waste cpu
...
function mod:SPELL_CAST_START(args)
	if args.spellId == 136216 then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_kjfd.mp3")--靠近分擔
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args.spellId == 136246 then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\range2.mp3")--2碼手雷
		showspellinfo()
	...
	elseif args.spellId == 136228 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\holdit.mp3")--自保
			VEM.Flash:Show(1, 0, 0)
		elseif mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_byt.mp3")--病原體出現
		end
	elseif args.spellId == 136245 then
		metabolicBoost = true
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_sljs.mp3")--加速
	...
	elseif args.spellId == 136218 then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\range5.mp3")--注意分散
		showspellinfo()
	elseif args.spellId == 140546 and args:IsPlayer() then
		VEM.Flash:Show(0, 1, 0)
		specWarnFullyMutated:Show()
		timerFullyMutated:Start()
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_tbwc.mp3")--完美突變
	end
end
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args.spellId == 140546 and args:IsPlayer() then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_bsjs.mp3")--變身結束
	end
end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 136248 and self:AntiSpam(2, 1) then--Pustule Eruption
		...
		if not self:IsDifficulty("lfr25") then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_zynx.mp3")--注意膿血
			if mod.Options.HudMAPF then
				for i = 1, VEM:GetNumGroupMembers() do
					...
					if inRange and inRange < 20 then
						FireMarkers[UnitName(uId)] = register(VEMHudMap:PlaceStaticMarkerOnPartyMember("highlight", UnitName(uId), 3, 2, 1, 1 ,1 ,0.8):Appear():RegisterForAlerts())
					end
				end
			end
		end
