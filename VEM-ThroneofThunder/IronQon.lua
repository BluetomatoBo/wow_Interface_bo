local mod	= VEM:NewMod(817, "VEM-ThroneofThunder", nil, 362)
...
local function notEligable(unit)
	...
	if (select(2, UnitClass(unit)) == "MONK") and (VEM.RangeCheck:GetDistance(unit, "boss2target") <= 15) then
		return true
	end
	return false
end
...
local function checkSpear()
	if UnitExists("boss1target") and not notEligable("boss1target") then--Boss 1 is looking at someone that isn't his highest threat or a tank (have to filter tanks cause he looks at them to cast impale, have to filter his highest threat in case it's not a tank, ie a healer)
		...
		local targetname = VEM:GetUnitFullName("boss1target")
...
function mod:OnCombatStart(delay)
	...
	sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_hyxt.mp3") --火焰形态
	--BH ADD END
	...
	if self:IsDifficulty("heroic10", "heroic25") then
		...
		if self.Options.InfoFrame then
			VEM.InfoFrame:SetHeader(GetSpellInfo(136193))
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args.spellId == 134647 then
		...
		self:Schedule(0.2, function()
			if morestack == 1 then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			...
			elseif morestack == 5 then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
			elseif morestack == 6 then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countsix.mp3")
			end
			morestack = 0
		end)
		]]
	elseif args.spellId == 137221 then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_rycz.mp3") --熔岩超載
	...
	elseif args.spellId == 136193 then
		if phase > 2 then
			if self.Options.HudLight and not FireMarkers[args.destName] then
				FireMarkers[args.destName] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("highlight", args.destName, 10, nil, 1, 1 ,1 ,0.8):Appear():RegisterForAlerts())
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args.spellId == 136192 then
		...
		if (args.destName == mod.Options.dispsetLight1) or (args.destName == mod.Options.dispsetLight2) then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\thanks.mp3")
		end
		if self.Options.SetIconOnLightningStorm and not self:IsDifficulty("lfr25") then
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args.spellId == 137226 then--Front, Right Shielded
		...
		if self.Options.SoundARAT then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\attmid.mp3")
		else
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_mq.mp3")
		end
	...
	elseif args.spellId == 137229 then--Back, Front Shielded
		...
		timerDeadZoneCD:Start()
		if self.Options.SoundARAT then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\attleft.mp3")
		else
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_mq.mp3")
		end
	...
	elseif args.spellId == 137230 then--Back, Left Shielded
		...
		timerDeadZoneCD:Start()
		if self.Options.SoundARAT then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\attright.mp3")
		else
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_mq.mp3")
		end
...
function mod:SPELL_SUMMON(args)
	if args.spellId == 134926 and phase < 4 then
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\spear.mp3") --投擲長矛
	end
end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	...
	elseif spellId == 50630 then--Eject All Passengers (heroic phase change trigger)
		...
		if cid == 68079 then--Ro'shak
			...
			updateHealthFrame()
			if self:IsDifficulty("heroic10", "heroic25") then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_sdxt.mp3") --閃電形态
			end
			...
			sndWOP:Schedule(47, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\wwsoon.mp3")
			sndWOP:Schedule(48, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
			sndWOP:Schedule(49, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			sndWOP:Schedule(50, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndWOP:Schedule(51, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			sndWOP:Schedule(52, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		elseif cid == 68080 then--Quet'zal
				...
				for i = 1, VEM:GetNumGroupMembers() do
					if UnitDebuff("raid"..i, GetSpellInfo(136193)) then
						if UnitName("raid"..i) ~= UnitName("player") then
							FireMarkers[UnitName("raid"..i)] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("highlight", UnitName("raid"..i), 10, nil, 1, 1 ,1 ,0.8):Appear():RegisterForAlerts())
						else
							FireMarkers[UnitName("raid"..i)] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("highlight", UnitName("raid"..i), 10, nil, 1, 1 ,1 ,0.4):Appear():RegisterForAlerts())
						end
			...
			updateHealthFrame()
			if self:IsDifficulty("heroic10", "heroic25") then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_bsxt.mp3") --冰霜形態
			end
			...
			sndWOP:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\wwsoon.mp3")
		...
		elseif cid == 68081 then--Dam'ren
			...
			if self:IsDifficulty("heroic10", "heroic25") then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ptwo.mp3") --2階段
			end
		end
	...
	elseif spellId == 137656 and self:AntiSpam(2, 1) then--Rushing Winds (Wind Storm end trigger)
		...
		if phase == 2 then
			...
			sndWOP:Schedule(65, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\wwsoon.mp3")
			sndWOP:Schedule(66, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
			sndWOP:Schedule(67, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			sndWOP:Schedule(68, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndWOP:Schedule(69, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			sndWOP:Schedule(70, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		end
...
function mod:UNIT_DIED(args)
	...
	if cid == 68079 then--Ro'shak
		...
		if self:IsDifficulty("heroic10", "heroic25") then--In heroic, all mounts die in phase 4.
			...
			sndWOP:Schedule(44.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\wwsoon.mp3")
			sndWOP:Schedule(45.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
			sndWOP:Schedule(46.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
			sndWOP:Schedule(47.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndWOP:Schedule(48.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			sndWOP:Schedule(49.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
