local mod	= VEM:NewMod(825, "VEM-ThroneofThunder", nil, 362)
...
function mod:OnCombatStart(delay)
	...
	sndWOP:Schedule(24, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\stompsoon.mp3")--準備踐踏
	...
	if VEM:GetRaidRank() > 0 and self.Options.SetIconOnTurtles and not VEM.Options.DontSetIcons then--You can set marks and you have icons turned on
		self:SendSync("IconCheck", UnitGUID("player"), tostring(VEM.Revision))
	end
end
...
function mod:SPELL_CAST_START(args)
	if args.spellId == 133939 then
		...
		sndAE:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
		sndAE:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
		sndAE:Cancel("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		sndAE:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\kickcast.mp3")--快打斷
	elseif args.spellId == 136294 then
		...
		sndXG:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_xwg.mp3")--小烏龜出現
	elseif args.spellId == 135251 then
		if UnitName("boss1target") == UnitName("player") then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_xxsy.mp3")--小心撕咬
		end
		warnBite:Show()
		timerBiteCD:Start()
	elseif args.spellId == 134920 then
		...
		sndWOP:Schedule(45, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\stompsoon.mp3")--準備踐踏
		if MyJS() then
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\defensive.mp3") --注意減傷
		else
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\stompstart.mp3")--踐踏開始
		end
		stomptime = GetTime()
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	if args.spellId == 133971 then--Shell Block (turtles dying and becoming kickable)
		...
		if VEM:GetRaidRank() > 0 and self.Options.ClearIconOnTurtles then
			for uId in VEM:GetGroupMembers() do
				local unitid = uId.."target"
				local guid = UnitGUID(unitid)
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 136685 then --Don't filter main tank, bat tank often taunts boss just before bats for vengeance, otherwise we lose threat to dps. Then main tank taunts back after bats spawn and we go get them, fully vengeanced (if you try to pick up bats without vengeance you will not hold aggro for shit)
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_bfcx.mp3")--蝙蝠出現
	end
end
...
function mod:OnSync(msg, guid, ver)
	...
	elseif msg == "aesoon" then
		if mod:AntiSpam(15, 10) then
			sndAE:Schedule(0.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\aesoon.mp3") --準備AE
			sndAE:Schedule(1, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")	
			sndAE:Schedule(2, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndAE:Schedule(3, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			sndAE:Schedule(4, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		end
	end
end