local mod	= VEM:NewMod(831, "VEM-ThroneofThunder", nil, 362)
...
local function LoopUnstableAnima()
	...
	if mod.Options.HudMAPAnima and UnstableAnimatarget then
		UnstableAnimaMarkers[UnstableAnimatarget] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("timer", UnstableAnimatarget, 8, 5, 1, 1, 0, 0.8):Appear():RegisterForAlerts():Rotate(360, 5.5):SetAlertColor(0, 0, 1, 0.5))
	end
end
...
function mod:SPELL_CAST_START(args)
	if args.spellId == 138321 then --造物材料
		...
		sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_zbdq.mp3") --準備打球
	...
	elseif args.spellId == 138338 then --召喚恐魔
		...
		if self:AntiSpam(2, 1) then
			if not mod:IsHealer() then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_xgkd.mp3") --小怪快打
			end
		end
	elseif args.spellId == 138339 then --召唤龟裂追踪者
		...
		if self:AntiSpam(2, 1) then
			if not mod:IsHealer() then
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_xgkd.mp3")
			end
		end
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args.spellId == 138334 then
		...
		if mod:IsTank() or mod:IsHealer() then
			sndStrike:Schedule(7, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndStrike:Schedule(8, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			sndStrike:Schedule(9, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		end
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	if args.spellId == 138297 then --不穩定的生命
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_smdn.mp3") --生命點你
			sndWOP:Schedule(0.5, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\movesoon.mp3")  --快跑位
			if self:IsDifficulty("heroic25") then
				...
				sndWOP:Schedule(7, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfive.mp3")
				sndWOP:Schedule(8, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countfour.mp3")
				sndWOP:Schedule(9, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
				sndWOP:Schedule(10, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
				sndWOP:Schedule(11, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
			end
		else
		...
		if self.Options.HudMAP then
			...
			FireMarkers[args.destName] = register(VEMHudMap:PlaceRangeMarkerOnPartyMember("highlight", args.destName, 3, 10, 1, 0 ,0 ,0.8):SetLabel(spelltext))
		end
		...
		if (lightcount == mylightnum) and (mod.Options.optDD == "nodd") then
			if UnitDebuff("player", GetSpellInfo(138372)) or UnitIsDeadOrGhost("player") then
				...
				sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\movesoon.mp3")
			end
		end
	...
	elseif args.spellId == 138288 then --不稳定的心能
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\ex_tt_xndn.mp3") --心能點你
		else
	...
	elseif args.spellId == 138332 then --灌输生命
		...
		if mod:IsTank() or mod:IsHealer() then
			sndStrike:Schedule(10-radenPower-3, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countthree.mp3")
			sndStrike:Schedule(10-radenPower-2, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\counttwo.mp3")
			sndStrike:Schedule(10-radenPower-1, "Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\countone.mp3")
		end
	end
end
