local mod	= VEM:NewMod(821, "VEM-ThroneofThunder", nil, 362)
...
local function showheadinfo()
	...
	if mod.Options.InfoFrame then
		...
		if mod:IsDifficulty("heroic10", "heroic25") then
			...
			VEM.InfoFrame:SetHeader(L.Behind.." ("..(Ramcount + 1).."/7)")
			VEM.InfoFrame:Show(3, "other", iceBehindcolor, iceinfob, venomBehindcolor, venominfob, fireBehindcolor, fireinfob)
		end
	end
end
...
function mod:SPELL_CAST_SUCCESS(args)
	if args.spellId == 140138 then
		...
		if self:AntiSpam(10, 5) then
			sndXL:Play("Interface\\AddOns\\VEM-Core\\extrasounds\\"..VEM.Options.CountdownVoice.."\\dragonnow.mp3")  --小龍出現
		end
